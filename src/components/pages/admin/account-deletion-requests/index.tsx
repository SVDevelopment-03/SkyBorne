...
import { columns, AccountDeletionRequestRow } from "./Column";
...
const AccountDeletionRequests = () => {
  ...
  const handleApprove = async (requestId: string) => {
    try {
      const response: any = await approveRequest(requestId).unwrap();
      const notifications = response?.notifications || {};

      if (notifications.emailSent || notifications.pushSent) {
        toast.success("Deletion request approved. User notified.");
      } else {
          ...
          <div className="flex flex-wrap items-center gap-2">
            {statusFilters.map((item) => (
              <Button
                ...
                className={status === item ? "capitalize" : "capitalize border-[#DCE5E0] text-[#494949] bg-white hover:bg-[#fbe6e6]"}
