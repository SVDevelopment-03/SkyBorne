...
import { Typography } from "./heading";
interface HasId {
  _id: string;
}
...
export function DataTable<TData extends HasId, TValue>({
  ...
  const table = useReactTable({
    data: data,
    columns,
      getRowId: (row) => row._id, // TS now understands this perfectly
        ...
        <TableBody>
          {isLoadingData ? (
            <tr className="w-full rounded-lg">
              {columns.map((column) => (
                <td key={ Math.random()} className="w-full first:rounded-tl-lg first:rounded-bl-lg last:rounded-tr-lg last:rounded-br-lg">
            ...
            table.getRowModel().rows.map((row) => (
              <TableRow
                ...
                className=" border-x-8 border-gray-700 hover:bg-gray-200 bg-[#F9F9F9] rounded-lg"
              >
                {row.getVisibleCells().map((cell) => (
                  <TableCell
                    key={cell.id}
                    className="py-2 px-5 font-satoshi-500 font-medium text-[1rem] leading-tight text-[#000000] first:rounded-tl-lg first:rounded-bl-lg last:rounded-tr-lg last:rounded-br-lg"
                  >
            ...
            <TableRow>
              <TableCell colSpan={columns.length} className="h-24 text-center">
                <Typography title="No Record yet"/>
